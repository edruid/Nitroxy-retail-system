	...
	<thead>
		<tr>
			...
			<th class="name_column">
				<a href="#"
					onclick="
						sort.sort(<?=$i++?>, sort.tagInsensitiveComparator);
						return false
					"
				>Namn</a>
			</th>
			...
			<th class="count_column">
				<a href="#" onclick="
					sort.sort(<?=$i++?>);
					return false
				">Lager</a>
			</th>
			<th class="total_value_column">
				<a href="#" onclick="
					sort.sort(<?=$i++?>);
					return false
				">Lagervärde</a>
			...
			<th class="sales_value">
				<a href="#" onclick="
					sort.sort(<?=$i++?>);
					return false
				">sålda 30 dag kr</a>
			</th>
			<th class="sales_revenue">
				<a href="#" onclick="
					sort.sort(<?=$i++?>);
					return false
				">vinst 30 dag kr</a>
	...
	<tbody>
		<?php foreach($products as $product): ?>
			...
			>
				...
				<td class="numeric price_column"><?=number($product->price)?> kr</td>
				<td class="numeric count_column"><?=$product->count?> st</td>
				<td class="numeric total_value_column">
					<?=number($product->count * $product->value)?> kr
				</td>
				<td
					class="
						numeric
						revenue
						<?=mark(($diff = $product->price-$product->value)<=0)?>
					"
				>
					<?=number($diff)?> kr
				</td>
				<td
					class="
						numeric
						revenue_p
						<?=mark(($markup=round($diff/$product->price*100))<20)?>
					"
				>
					<?=$markup?> %
				</td>
				<td class="numeric threshold">
					<?php if($product->inventory_threshold): ?>
						<?=$product->inventory_threshold?> st
					<?php endif ?>
				</td>
				<td
					class="
						...
						threshold_diff
						<?=mark(
							($t_diff=$product->count-$product->inventory_threshold)<=0 &&
							$product->inventory_threshold != null
						)?>
					"
				>
					<?php if($product->inventory_threshold>0): ?>
						<?=$t_diff?> st
					<?php endif ?>
				</td>
				<td class="numeric sales">
					<?=
						$count = TransactionContent::sum('count', array(
				...
				<td class="numeric sales_value">
					<?=number(
						$sold = TransactionContent::sum('amount', array(
							'product_id' => $product->id,
							'Transaction.timestamp:>' => date('Y-m-d', time()-60*60*24*30)
						))
					)?> kr
