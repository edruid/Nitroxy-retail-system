	...
	<tbody>
		<? foreach($products as $product): ?>
			<tr>
				...
				<td class="numeric sales"><?=TransactionContent::sum('count', array('product_id' => $product->id, 'Transaction.timestamp:>' => date('Y-m-d', time()-60*60*24*30)))?> st</td>
				<td class="numeric sales_value"><?=number(TransactionContent::sum('amount', array('product_id' => $product->id, 'Transaction.timestamp:>' => date('Y-m-d', time()-60*60*24*30))))?> kr</td>
			</tr>
		<? endforeach ?>
	</tbody>
</table>
